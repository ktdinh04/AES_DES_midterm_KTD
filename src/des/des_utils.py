...
def pad(data):
    """Pad the data to be a multiple of 8 bytes using PKCS#5 padding"""
...
def split_blocks(block):
    """Split the block into two equal halves"""
    middle = len(block) // 2
    return block[:middle], block[middle:]

def shift_left(key, shifts):
    """Circular left shift the key by the specified number of bits"""
    return key[shifts:] + key[:shifts]

def expand(block):
    """Expand the 32-bit block to 48-bits using the E table"""
...
def substitution(block):
    """Apply S-box substitution on the 48-bit block, returning a 32-bit block"""
    result = ""
...
def permutation(block):
    """Apply permutation using the P table"""
...
def generate_subkeys(key):
    ...
    left, right = split_blocks(key_bits)
    
    subkeys = []
    for i in range(16):
        # Perform left shifts
        left = shift_left(left, KEY_SHIFTS[i])
        right = shift_left(right, KEY_SHIFTS[i])
